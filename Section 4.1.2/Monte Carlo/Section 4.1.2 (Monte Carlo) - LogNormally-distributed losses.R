...
# R Code to calculate the ruin probability when each participant's claim sizes are LogNormally distributed - Section 4.1.2
# Note that, this code estimates the ruin probability via Monte Carlo simulation. Hence, it serves to validate our results
# using pollaczek-khinchine's formula.
...
# n: a numeric integer that provides the total number of random numbers with a LogNormal 
# distribution to be generated.
# probs: a vector containing the weights allocated to each of the LogNormal distributions.
...
SimulateRuinProbabilityPool <- function(InitialCapital = 0, c = 5.6, lambda = lambda_1 + lambda_2 + lambda_3, probs = c(2/3, 1/3), mus = c(0, 0.5, -0.3), sigmas = c(0.25, 0.25, 0.25), M_is = c(8/15, 8/15, 8/15), T_max = 1000, Simulations = 10000) {
  ...
  for (k in 1:Simulations) {
    ...
    claim_times <- sort(runif(n_claims, 0, T_max))  # we generate the exact times when the n_claims occur; these times are uniformly distributed over the time interval [0, T]. That is, the have the same probability of occurring at any time in this interval.
    claim_sizes <- rmixture_scaled_lognormals(n_claims, probs, mus, sigmas, M_is)$x # we generate the claim sizes which will follow a mixture of scaled LogNormal distributions.
    capital <- InitialCapital + c * claim_times - cumsum(claim_sizes) # we calculate the capital values immediately after each claim
    ruin_flags[k] <- any(capital <= 0) # since ruin can only occur at claim times, then we just need to now check if capital <= 0 for any of its entries
  }
  mean(ruin_flags) # lastly, the estimate of the ruin probability is given by the sum of TRUEs divided by the total number of simulations; in other words, we can also calculate this by estimating the mean of the ruin_flags vector
...
# The function below is in charge of simulating the ruin probability for the Cramér-Lundberg model 
# for the pooled fund. The inputs for this function are:
# InitialCapital: a number indicating the initial capital for participant "i" (\kappa_{i})
# c: the premium rate for participant "i" (c_{i} = (1 + \eta) * E[S_{i,1}] = (1 + \eta) * \lambda_{i} * E[Y_{i,1}])
# lambda: the intensity of the compound Poission process. Recall that this parameter somehow describes us the frequency
# of the losses experienced in the pool (\lambda = \lambda_{1} + \lambda_{2} + ... + \lambda_{n})
...
SimulateRuinProbabilityPooledFund <- function(InitialCapital = 0, c = 5.6, lambda = lambda_1 + lambda_2 + lambda_3, probs = c(2/3, 1/3), mus = c(0, 0.5, -0.3), sigmas = c(0.25, 0.25, 0.25), T_max = 1000, Simulations = 10000) {
  ...
  for (k in 1:Simulations) {
    ...
    claim_times <- sort(runif(n_claims, 0, T_max))  # we generate the exact times when the n_claims occur; these times are uniformly distributed over the time interval [0, T]. That is, the have the same probability of occurring at any time in this interval.
    claim_sizes <- rmixtureoflognormals(n_claims, probs, mus, sigmas)$x # we generate the claim sizes which will follow a mixture of Lognormal distributions.
    capital <- InitialCapital + c * claim_times - cumsum(claim_sizes) # we calculate the capital values immediately after each claim
    ruin_flags[k] <- any(capital <= 0) # since ruin can only occur at claim times, then we just need to now check if capital <= 0 for any of its entries
  }
  mean(ruin_flags) # lastly, the estimate of the ruin probability is given by the sum of TRUEs divided by the total number of simulations; in other words, we can also calculate this by estimating the mean of the ruin_flags vector
...
# Example 1. Estimation of ruin probability for participant 1
SimulateRuinProbabilityPooledFund(InitialCapital = 0.58, c = c, lambda = lambda, probs = probs, mus = mus, sigmas = sigmas, T_max = T_max, Simulations = Simulations)

# Use parallel computing
cores <- detectCores() - 1
cl <- makeCluster(cores)
clusterExport(cl, varlist = c("SimulateRuinProbabilityPool", "SimulateRuinProbabilityIndividual", "SimulateRuinProbabilityPooledFund", "rmixtureoflognormals", "rmixture_scaled_lognormals", "Simulations", "T_max", "eta", "lambda_1", "mu_1", "sigma_1", "c_1", "MP_1s", "ALT_1s", "lambda_2", "mu_2", "sigma_2", "c_2", "MP_2s", "ALT_2s", "lambda_3", "mu_3", "sigma_3", "c_3", "MP_3s", "ALT_3s", "lambda", "c", "probs", "mus", "sigmas"))

ruin_probs_pooled_fund <- parSapply(cl, initial_capitals, function(IC) {SimulateRuinProbabilityPooledFund(InitialCapital = IC, c = c, lambda = lambda, probs = probs, mus = mus, sigmas = sigmas, T_max = T_max, Simulations = Simulations)})
...
lines(initial_capitals, ruin_probs_pooled_fund, lwd = 2, lty = 5, col = "purple")
...
tikz('PlotInfiniteTimeRuinProbabilitySection412 (Monte Carlo) - All Participants ALT.tex', standAlone = TRUE, width = 4, height = 4, packages = c("\\usepackage{tikz}", "\\usepackage[active,tightpage,psfixbb]{preview}", "\\PreviewEnvironment{pgfpicture}", "\\setlength\\PreviewBorder{0pt}", "\\usepackage{amssymb}", "\\usepackage{scalerel}", "\\usepackage{amsmath}"))
my.expressions <- c("$\\psi_{{\\scaleto{1}{3pt}}}^{{\\scaleto{ {\\fontfamily{qcr}\\selectfont \\text{POOL}}}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{2}{3pt}}}^{{\\scaleto{ {\\fontfamily{qcr}\\selectfont \\text{POOL}}}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{3}{3pt}}}^{{\\scaleto{ {\\fontfamily{qcr}\\selectfont \\text{POOL}}}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{1}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{2}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{3}{3pt}}}\\left(\\kappa\\right)$", "$\\psi\\left(\\kappa\\right)$")
par(mgp = c(2.5, 1, 0), mar = c(3.5, 3.5, 1, 1) + 0.1)
