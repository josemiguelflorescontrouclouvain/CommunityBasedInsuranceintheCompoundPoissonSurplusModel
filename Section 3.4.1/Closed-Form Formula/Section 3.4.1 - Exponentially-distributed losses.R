...
# R Code to calculate the ruin probability when each participant's claim sizes are Exponentially distributed - Section 3.4.1
...
lambda_i <- c(2, 1, 3) # Frequencies (intensities) for each participant
lambda <- sum(lambda_i) # Sum of all Frequencies (intensities)
alpha_i  <- c(1/2, 2, 1) # Rate parameter of the exponential distribution for each participant
...
ALT_1s <- c(0.8, 0.1, 0.25) # Alternative (ALT) risk-sharing scheme for participant 1
ALT_2s <- c(0.0375, 0.400, 0.050) # Alternative (ALT) risk-sharing scheme for participant 2
ALT_3s <- c(0.1625, 0.5, 0.7) # Alternative (ALT) risk-sharing scheme for participant 3
...
# Note that this is required for each individual.
...
initial_capitals <- seq(0, 40, by = 0.01)
...
MyColors <- pal_jco()(4)
MyLines <-seq(from = 1, to = 4, by = 1)
plot(initial_capitals, ruin_probs_2s, type = "l", lwd = 2, lty = MyLines[1], col = MyColors[1], xaxs = "i", yaxs = "i", xlim = c(min(initial_capitals), 10), ylim = c(0, 1), xlab = "$\\kappa, \\kappa_{2}$", ylab = "Ruin Probability")
...
MyColors <- pal_jco()(4)
MyLines <-seq(from = 1, to = 4, by = 1)
plot(initial_capitals, ruin_probs_3s, type = "l", lwd = 2, lty = MyLines[1], col = MyColors[1], xaxs = "i", yaxs = "i", xlim = c(min(initial_capitals), 20), ylim = c(0, 1), xlab = "$\\kappa, \\kappa_{3}$", ylab = "Ruin Probability")
