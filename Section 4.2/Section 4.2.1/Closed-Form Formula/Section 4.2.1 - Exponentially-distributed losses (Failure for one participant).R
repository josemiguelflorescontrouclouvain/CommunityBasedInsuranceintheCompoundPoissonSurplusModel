...
# R Code to calculate the ruin probability when each participant's claim sizes are Exponentially distributed - Section 4.2.1
...
lambda_i <- c(2, 5, 40) # Frequencies (intensities) for each participant
lambda <- sum(lambda_i) # Sum of all Frequencies (intensities)
alpha_i  <- c(1/10, 1/4, 2) # Rate parameter of the exponential distribution for each participant
...
ALT_1s <- c(0.5, 0.3, 0.2) # Alternative (ALT) risk-sharing scheme for participant 1
ALT_2s <- c(0.1, 0.6, 0.3) # Alternative (ALT) risk-sharing scheme for participant 2
ALT_3s <- c(0.4, 0.1, 0.5) # Alternative (ALT) risk-sharing scheme for participant 3
...
cat("      Pool mean:", sum(lambda_i * MP_1s * mu_i), " | Individual mean:", lambda_i[1] * mu_i[1], "\n")
cat("    Alternative risk-sharing rule:\n")
cat("      Pool mean:", sum(lambda_i * ALT_1s * mu_i), " | Individual mean:", lambda_i[1] * mu_i[1], "\n")

# Participant 2
cat("Participant 2:\n")
cat("  Fairness check:\n")
cat("    Mean-proportional risk-sharing rule:\n")
cat("      Pool mean:", sum(lambda_i * MP_2s * mu_i), " | Individual mean:", lambda_i[2] * mu_i[2], "\n")
cat("    Alternative risk-sharing rule:\n")
cat("      Pool mean:", sum(lambda_i * ALT_2s * mu_i), " | Individual mean:", lambda_i[2] * mu_i[2], "\n")

# Participant 3
cat("Participant 3:\n")
cat("  Fairness check:\n")
cat("    Mean-proportional risk-sharing rule:\n")
cat("      Pool mean:", sum(lambda_i * MP_3s * mu_i), " | Individual mean:", lambda_i[3] * mu_i[3], "\n")
cat("    Alternative risk-sharing rule:\n")
cat("      Pool mean:", sum(lambda_i * ALT_3s * mu_i), " | Individual mean:", lambda_i[3] * mu_i[3], "\n")
...
cat("      The required inequality is: ", MP_1s[1] * mu_i[1] <= mu_i[1], "\n")
cat("    Capacity Constraint check for a_12:\n")
cat("      The required inequality is: ", MP_1s[2] * mu_i[2] <= mu_i[1], "\n")
cat("    Capacity Constraint check for a_13:\n")
cat("      The required inequality is: ", MP_1s[3] * mu_i[3] <= mu_i[1], "\n")
cat("  Alternative risk-sharing rule:\n")
cat("    Capacity Constraint check for a_11:\n")
cat("      The required inequality is: ", ALT_1s[1] * mu_i[1] <= mu_i[1], "\n")
cat("    Capacity Constraint check for a_12:\n")
cat("      The required inequality is: ", ALT_1s[2] * mu_i[2] <= mu_i[1], "\n")
cat("    Capacity Constraint check for a_13:\n")
cat("      The required inequality is: ", ALT_1s[3] * mu_i[3] <= mu_i[1], "\n")

# Participant 2
cat("Participant 2:\n")
cat("  Mean-proportional risk-sharing rule:\n")
cat("    Capacity Constraint check for a_21:\n")
cat("      The required inequality is: ", MP_2s[1] * mu_i[1] <= mu_i[2], "\n")
cat("    Capacity Constraint check for a_22:\n")
cat("      The required inequality is: ", MP_2s[2] *mu_i[2] <= mu_i[2], "\n")
cat("    Capacity Constraint check for a_23:\n")
cat("      The required inequality is: ", MP_2s[3] * mu_i[3] <= mu_i[2], "\n")
cat("  Alternative risk-sharing rule:\n")
cat("    Capacity Constraint check for a_21:\n")
cat("      The required inequality is: ", ALT_2s[1] * mu_i[1] <= mu_i[2], "\n")
cat("    Capacity Constraint check for a_22:\n")
cat("      The required inequality is: ", ALT_2s[2] * mu_i[2] <= mu_i[2], "\n")
cat("    Capacity Constraint check for a_23:\n")
cat("      The required inequality is: ", ALT_2s[3] * mu_i[3] <= mu_i[2], "\n")

# Participant 3
cat("Participant 3:\n")
cat("  Mean-proportional risk-sharing rule:\n")
cat("    Capacity Constraint check for a_31:\n")
cat("      The required inequality is: ", MP_3s[1] * mu_i[1] <= mu_i[3], "\n")
cat("    Capacity Constraint check for a_32:\n")
cat("      The required inequality is: ", MP_3s[2] * mu_i[2] <= mu_i[3], "\n")
cat("    Capacity Constraint check for a_33:\n")
cat("      The required inequality is: ", MP_3s[3] * mu_i[3] <= mu_i[3], "\n")
cat("  Alternative risk-sharing rule:\n")
cat("    Capacity Constraint check for a_31:\n")
cat("      The required inequality is: ", ALT_3s[1] * mu_i[1] <= mu_i[3], "\n")
cat("    Capacity Constraint check for a_32:\n")
cat("      The required inequality is: ", ALT_3s[2] * mu_i[2] <= mu_i[3], "\n")
cat("    Capacity Constraint check for a_33:\n")
cat("      The required inequality is: ", ALT_3s[3] * mu_i[3] <= mu_i[3], "\n")

# We calculate the mean of Y_{k} for the pooled fund.

muY <- sum(lambda_i/lambda * 1/alpha_i) # For pooled fund

# We calculate the mean of Z_{i,k} for each participant
...
# We define G_Y(r), the moment generating function of the random variable Y_{k} (which follows a mixture of Exponential distributions with rate parameters alpha_{i}).

# Note that this is required for the pooled fund.

G_Y <- function(r, lambda, lambda_vec, alpha_vec) {
  sum_terms <- sum((alpha_vec * lambda_vec) / (alpha_vec - r))
...
# First, for the pooled fund.

denominator_func_Y <- function(r, eta, mu_Y, lambda, lambda_vec, alpha_vec) {
  gy_r <- G_Y(r, lambda, lambda_vec, alpha_vec)
  val <- 1 + (1 + eta) * mu_Y * r - gy_r
  return(val)
}

# Second, for each individual.

denominator_func_Zi <- function(r, eta, mu_Zi, lambda, lambda_vec, beta_vec) {
  gz_r <- G_Zi(r, lambda, lambda_vec, beta_vec)
  val <- 1 + (1 + eta) * mu_Zi * r - gz_r
  return(val)
}

# For the pooled fund, we find the roots: r_{1} and r_{2}.

# First, we sort alpha_i to identify the intervals between singularities.

sorted_alphas <- sort(alpha_i)

# For each individual, we find the roots: r_{1}, r_{2} and r_{3}.
...
intervals_alphas <- list(
  c(1e-10, sorted_alphas[1] - 1e-10), # Interval for r_1
  c(sorted_alphas[1] + 1e-10, sorted_alphas[2] - 1e-10), # Interval for r_2
  c(sorted_alphas[2] + 1e-10, sorted_alphas[3] - 1e-10)  # Interval for r_3
)
...
# Lastly, we compute all roots for the pooled fund and for each of the three participants

for(i in 1:3) {
  resalphas <- uniroot(
    f = denominator_func_Y,
    interval = intervals_alphas[[i]],
  ...
  roots_alphas[i] <- resalphas$root
}

for(i in 1:3) {
  resMP_1s <- uniroot(
    f = denominator_func_Zi,
...
# Output all roots for the pooled fund and for each of the three participants.

print(paste("Root 1 for pooled fund:", roots_alphas[1]))
print(paste("Root 2 for pooled fund:", roots_alphas[2]))
print(paste("Root 3 for pooled fund:", roots_alphas[3]))
...
find_coefficients_Y <- function(roots, eta, mu_Y, lambda, lambda_vec, alpha_vec) {
  
  dG_dr <- function(r) {
    sum((alpha_vec * lambda_vec) / (alpha_vec - r)^2) / lambda
  }
  ...
  for (i in seq_along(roots)) {
    ...
    gy_ri <- G_Y(ri, lambda, lambda_vec, alpha_vec)
    num <- (eta * (gy_ri - 1)) / (1 + eta)
    
    den_prime <- ((1 + eta) * mu_Y) - dG_dr(ri)
...
find_coefficients_Zi <- function(roots, eta, mu_Zi, lambda, lambda_vec, beta_vec) {
...
Ci_alphas <- find_coefficients_Y(roots_alphas, eta, muY, lambda, lambda_i, alpha_i) # Run for pooled fund

CiMP_1s <- find_coefficients_Zi(rootsMP_1s, eta, muZMP_1s, lambda, lambda_i, betaMP_1s) # Run for participant 1
CiMP_2s <- find_coefficients_Zi(rootsMP_2s, eta, muZMP_2s, lambda, lambda_i, betaMP_2s) # Run for participant 2
CiMP_3s <- find_coefficients_Zi(rootsMP_3s, eta, muZMP_3s, lambda, lambda_i, betaMP_3s) # Run for participant 3

CiALT_1s <- find_coefficients_Zi(rootsALT_1s, eta, muZALT_1s, lambda, lambda_i, betaALT_1s) # Run for participant 1
CiALT_2s <- find_coefficients_Zi(rootsALT_2s, eta, muZALT_2s, lambda, lambda_i, betaALT_2s) # Run for participant 2
CiALT_3s <- find_coefficients_Zi(rootsALT_3s, eta, muZALT_3s, lambda, lambda_i, betaALT_3s) # Run for participant 3

# Output all coefficients for the pooled fund and for each of the three participants.

print(paste("Coefficient 1 for pooled fund:", Ci_alphas[1]))
print(paste("Coefficient 2 for pooled fund:", Ci_alphas[2]))
print(paste("Coefficient 3 for pooled fund:", Ci_alphas[3]))
...
initial_capitals <- seq(0, 50, by = 0.01)
...
# We calculate the ruin probabilities for the pooled fund.

ruin_probs_alphas <- sapply(initial_capitals, function(kappa) {ruin_probability_mixture_exponentials(kappa, roots_alphas, Ci_alphas)})
...
tikz('PlotInfiniteTimeRuinProbabilitySection421 - Participant 1.tex', standAlone = TRUE, width = 4, height = 4, packages = c("\\usepackage{tikz}", "\\usepackage[active,tightpage,psfixbb]{preview}", "\\PreviewEnvironment{pgfpicture}", "\\setlength\\PreviewBorder{0pt}", "\\usepackage{amssymb}", "\\usepackage{scalerel}", "\\usepackage{amsmath}"))
par(mgp = c(2.5, 1, 0), mar = c(3.5, 3.5, 1, 1) + 0.1)
MyColors <- pal_jco()(4)
MyLines <-seq(from = 1, to = 4, by = 1)
plot(initial_capitals, ruin_probs_1s, type = "l", lwd = 2, lty = MyLines[1], col = MyColors[1], xaxs = "i", yaxs = "i", xlim = c(min(initial_capitals), max(initial_capitals)), ylim = c(0, 1), xlab = "$\\kappa, \\kappa_{1}$", ylab = "Ruin Probability")
...
lines(initial_capitals, ruin_probs_alphas, lwd = 2, lty = MyLines[4], col = MyColors[4])
...
tikz('PlotInfiniteTimeRuinProbabilitySection421 - Participant 2.tex', standAlone = TRUE, width = 4, height = 4, packages = c("\\usepackage{tikz}", "\\usepackage[active,tightpage,psfixbb]{preview}", "\\PreviewEnvironment{pgfpicture}", "\\setlength\\PreviewBorder{0pt}", "\\usepackage{amssymb}", "\\usepackage{scalerel}", "\\usepackage{amsmath}"))
par(mgp = c(2.5, 1, 0), mar = c(3.5, 3.5, 1, 1) + 0.1)
MyColors <- pal_jco()(4)
MyLines <-seq(from = 1, to = 4, by = 1)
plot(initial_capitals, ruin_probs_2s, type = "l", lwd = 2, lty = MyLines[1], col = MyColors[1], xaxs = "i", yaxs = "i", xlim = c(min(initial_capitals), 30), ylim = c(0, 1), xlab = "$\\kappa, \\kappa_{2}$", ylab = "Ruin Probability")
...
tikz('PlotInfiniteTimeRuinProbabilitySection421 - Participant 3.tex', standAlone = TRUE, width = 4, height = 4, packages = c("\\usepackage{tikz}", "\\usepackage[active,tightpage,psfixbb]{preview}", "\\PreviewEnvironment{pgfpicture}", "\\setlength\\PreviewBorder{0pt}", "\\usepackage{amssymb}", "\\usepackage{scalerel}", "\\usepackage{amsmath}"))
par(mgp = c(2.5, 1, 0), mar = c(3.5, 3.5, 1, 1) + 0.1)
MyColors <- pal_jco()(4)
MyLines <-seq(from = 1, to = 4, by = 1)
plot(initial_capitals, ruin_probs_3s, type = "l", lwd = 2, lty = MyLines[1], col = MyColors[1], xaxs = "i", yaxs = "i", xlim = c(min(initial_capitals), 30), ylim = c(0, 1), xlab = "$\\kappa, \\kappa_{3}$", ylab = "Ruin Probability")
lines(initial_capitals, ruin_probsMP_2s, lwd = 2, lty = MyLines[2], col = MyColors[2])
lines(initial_capitals, ruin_probsALT_2s, lwd = 2, lty = MyLines[3], col = MyColors[3])
lines(initial_capitals, ruin_probs_alphas, lwd = 2, lty = MyLines[4], col = MyColors[4])
...
tikz('PlotInfiniteTimeRuinProbabilitySection421 - All Participants MP.tex', standAlone = TRUE, width = 4, height = 4, packages = c("\\usepackage{tikz}", "\\usepackage[active,tightpage,psfixbb]{preview}", "\\PreviewEnvironment{pgfpicture}", "\\setlength\\PreviewBorder{0pt}", "\\usepackage{amssymb}", "\\usepackage{scalerel}", "\\usepackage{amsmath}"))
...
lines(initial_capitals, ruin_probs_2s, lwd = 2, lty = 3, col = "gray")
lines(initial_capitals, ruin_probs_3s, lwd = 2, lty = 4, col = "brown")
lines(initial_capitals, ruin_probs_alphas, lwd = 2, lty = 5, col = "purple")
...
tikz('PlotInfiniteTimeRuinProbabilitySection421 - All Participants ALT.tex', standAlone = TRUE, width = 4, height = 4, packages = c("\\usepackage{tikz}", "\\usepackage[active,tightpage,psfixbb]{preview}", "\\PreviewEnvironment{pgfpicture}", "\\setlength\\PreviewBorder{0pt}", "\\usepackage{amssymb}", "\\usepackage{scalerel}", "\\usepackage{amsmath}"))
my.expressions <- c("$\\psi_{{\\scaleto{1}{3pt}}}^{{\\scaleto{ {\\fontfamily{qcr}\\selectfont \\text{POOL}}}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{2}{3pt}}}^{{\\scaleto{ {\\fontfamily{qcr}\\selectfont \\text{POOL}}}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{3}{3pt}}}^{{\\scaleto{ {\\fontfamily{qcr}\\selectfont \\text{POOL}}}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{1}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{2}{3pt}}}\\left(\\kappa\\right)$", "$\\psi_{{\\scaleto{3}{3pt}}}\\left(\\kappa\\right)$", "$\\psi\\left(\\kappa\\right)$")
par(mgp = c(2.5, 1, 0), mar = c(3.5, 3.5, 1, 1) + 0.1)
